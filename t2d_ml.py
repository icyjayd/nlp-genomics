...
if __name__=="__main__":
    ...
    parser.add_argument("--lr_penalty", choices=[None, 'l2'])
...
#         key_df = pd.concat([key_df.iloc[:idx], key_df.iloc[idx + 1:]])
    ...
    if model == "rf":
        ...
        clf = LogisticRegression(penalty=lr_penalty, max_iter=lr_max_iters)
    elif model == "knn":
        clf = KNeighborsClassifier(n_neighbors = round(np.sqrt(len(X_train))))
    elif model == "xgboost":
        clf = xgb.XGBClassifier(n_estimators =100, eta=0.01)
    ...
    joblib.dump(classifier, os.path.join(data_path, f"{model_type}_svd_{svd_components}_{contigs}_bpe_new_{bpe_vocab}_{day}.pkl"))
    joblib.dump(X_train, os.path.join(data_path, f"X_train_{model_type}_svd_{svd_components}_{contigs}_bpe_new_{bpe_vocab}_{day}.pkl"))
    joblib.dump(X_test, os.path.join(data_path, f"X_test_{model_type}_svd_{svd_components}_{contigs}_bpe_new_{bpe_vocab}_{day}.pkl"))
    joblib.dump(y_train, os.path.join(data_path, f"y_train_{model_type}_svd_{svd_components}_{contigs}_bpe_new_{bpe_vocab}_{day}.pkl"))
    joblib.dump(y_test, os.path.join(data_path, f"y_test_{model_type}_svd_{svd_components}_{contigs}_bpe_new_{bpe_vocab}_{day}.pkl"))
    h5file.close()
