...
def train(model, device, train_loader, optimizer, epoch, loss_fn, log_interval=1000):
    """Training loop."""
    model.train()
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if epoch ==0 and batch_idx == 0:
            for rank in range(0, 4):
                ...
                print("length of train_loader.dataset:", len(train_loader.dataset), "length of train_loader:", len(train_loader))
...
def prepare_data(rank, world_size, data, path_to_db, label_col, batch_size=32, pin_memory=False, num_workers=0):
    train_data, val_data = train_test_split(data, test_size=0.2)
    val_data, test_data = train_test_split(val_data, test_size=0.5)

    class_dict = dict(zip(data['family'].unique(), np.arange(len(data['family'].unique())))) ###necessary to define class dict to ensure label fidelity
...
def main(rank, world_size):
    ...
    train_loader, val_loader, test_loader = prepare_data(rank, world_size, samples, path_to_db=path_to_db, label_col=label_col,
    ...
    for epoch in range(num_epochs):
        ...
        train(model, rank, train_loader, optimizer, epoch, loss_fn)
