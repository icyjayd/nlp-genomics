...
def _init_weights(module, n_layer, initializer_range=0.02, rescale_prenorm_residual=True,
    ...
    if rescale_prenorm_residual:
        ...
        #   > the weights of residual layers at initialization by a factor of 1/vN where N is the # of residual layers.
...
def train(model, device, train_loader, optimizer, epoch, loss_fn, log_interval=10000):
    """Training loop."""
    model.train()
    ...
    torch.save({'model_state_dict':model.state_dict(), 'optimizer_state_dict':optimizer.state_dict(),'loss':loss}, '/gpfs/scratch/jic286/JohnsonLab/hyena-model-checkpoints/hyena-bpe-2_families-species_split-epoch_{}-24-03-02.pt'.format(epoch))
...
def split_by_species(sample, split):
    val_counts = sample['species'].value_counts()
    total = 0
    for i in range(len(val_counts)):
        total += val_counts[i]
        if float(total)/float(sum(val_counts)) >= split:
            break
    train_set = sample[sample['species'].isin(val_counts.index[:i+1])]
    test_set = sample[sample['species'].isin(val_counts.index[i+1:])]
...
def run_train():
    ...
    for i in range(n_classes):
        train_data, val_data = split_by_species(samples[samples['family']==family_counts.index[i]], 0.8)
        train_samples.append(train_data)
        val_data, test_data = split_by_species(val_data, 0.5)
