...
if __name__=="__main__":
    ...
    parser.add_argument("model", choices=['rf', 'svm', 'lr', 'knn'])
    ...
    parser.add_argument("--save_path", default = "/gpfs/scratch/jic286/JohnsonLab/alzheimers_model_data")
    ...
    if model !='lr':
        print("Warning: lr not selected as model. Lr penalty will not be used")
    else:
        print("lr penalty:", lr_penalty)
    if model != 'rf':
        print("Warning: rf not selected as model. N_estimators will not be used")
    else:
    ...
    h5file = open_file(os.path.join(path, contigs + "_tokenizations_" + threshold + bpe_vocab + ".h5"), 'r')
...
#        key_df = key_df.iloc[:-1]
    ...
    if model == "rf":
        clf = RandomForestClassifier(bootstrap= False, criterion= 'entropy', n_estimators= n_estimators,)
    elif model == "svm":
        clf = SVC()
    elif model == "lr":
    ...
    joblib.dump(classifier, os.path.join(data_path, f"{model_type}_svd_{svd_components}_{contigs}_bpe_{bpe_vocab}_{day}.pkl"))
    joblib.dump(X_train, os.path.join(data_path, f"X_train_{model_type}_svd_{svd_components}_{contigs}_bpe_{bpe_vocab}_{day}.pkl"))
    joblib.dump(X_test, os.path.join(data_path, f"X_test_{model_type}_svd_{svd_components}_{contigs}_bpe_{bpe_vocab}_{day}.pkl"))
    joblib.dump(y_train, os.path.join(data_path, f"y_train_{model_type}_svd_{svd_components}_{contigs}_bpe_{bpe_vocab}_{day}.pkl"))
    joblib.dump(y_test, os.path.join(data_path, f"y_test_{model_type}_svd_{svd_components}_{contigs}_bpe_{bpe_vocab}_{day}.pkl"))
    h5file.close()
