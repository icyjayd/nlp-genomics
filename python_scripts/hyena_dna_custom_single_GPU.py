...
samples = all_species[all_species['family'].isin(family_counts.index[2:n_classes+2])]
...
def train(model, device, train_loader, optimizer, epoch, loss_fn, day, log_interval=4000):
    """Training loop."""
    model.train()
    ...
    torch.save({'model_state_dict':model.state_dict(), 'optimizer_state_dict':optimizer.state_dict(),'loss':loss},
               '/gpfs/scratch/jic286/JohnsonLab/hyena-model-checkpoints/hyena-bpe-4_families-genus_split-epoch_{}-{}.pt'.format(epoch, day))
