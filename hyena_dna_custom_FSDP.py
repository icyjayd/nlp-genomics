...
def train(model, rank, world_size, train_loader, optimizer, epoch, loss_fn, class_num, log_interval=10):
    """Training loop."""
    model.train()
...
#    train_loader.sampler.set_epoch(epoch)
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        ddp_loss[1]+= len(data)
        if ddp_loss[1] % log_interval == 0 and rank == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f} | Elapsed time: {}'.format(
                epoch, ddp_loss[1], len(train_loader),
                100. * ddp_loss[1] / len(train_loader), ddp_loss[0]/ddp_loss[1],
            time.time() - start_time), flush=True)
    dist.all_reduce(ddp_loss, op = dist.ReduceOp.SUM)
    dist.barrier()
    if rank==0:
        torch.save({'model_state_dict':model.state_dict(), 'optimizer_state_dict':optimizer.state_dict(),'loss':loss},
                   '/gpfs/scratch/jic286/JohnsonLab/hyena-model-checkpoints/hyena-bpe-{}_families-FSDP-test-{}-epoch-{}.pt'.format(class_num, day, epoch))

    print("Epoch {} complete, elapsed time: {}".format(time.time() - start_time), flush=True)
        #torch.empty_cache()
...
def run_train(rank, world_size):
    ...
    class_num = 2
    print(family_counts[2:class_num+2], flush=True)
    samples = all_species[all_species['family'].isin(family_counts.index[:class_num+2])]
    train_loader, val_loader, test_loader = prepare_data(samples, rank, world_size, samples,path_to_db,label_col, n_classes = class_num,batch_size=batch_size,
    ...
    for epoch in range(num_epochs):
        train(model, rank, world_size, train_loader, optimizer, epoch, loss_fn, class_num)
        print("train loop done, testing now", flush=True)
